...
const HouseDetail = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // House fetch
                const houseResponse = await axios.get(`http://localhost:8080/api/houses/${rentalId}`, {
                ...
                // Review fetch
                const reviewsResponse = await axios.get(`http://localhost:8080/api/reviews/houses/${rentalId}`, {
                    withCredentials: true,
                    headers: {
                        "Content-Type": "application/json",
                    },
                ...
                if (Array.isArray(reviewsResponse.data)) {
                    ...
                    // 예외 처리
                    console.error("Invalid data format for reviews:", reviewsResponse.data);
                }
    ...
    const addBookmark = async () => {
        try {
            await axios.post(`http://localhost:8080/api/bookmarks/${rentalId}`, null, {
                withCredentials: true,
                headers: {
                    "Content-Type": "application/json",
                },
    ...
    const handleReservation = async () => {
        try {
            ...
            await axios.post(`http://localhost:8080/api/reservations/customer/${rentalId}`, reservationData, {
                withCredentials: true,
                headers: {
                    "Content-Type": "application/json",
            ...
            <div className="title flex flex-col">
                ...
                <div className="flex flex-row">
                    ...
                    {isLoggedInLocalStorage ? (
                        <button className="bookmark px-8 pb-4 text-sm justify-end ml-auto" onClick={addBookmark}>
            ...
            <div className="flex">
                ...
                <div className="p-8 flex-2 justify-center items-center border rounded-md mr-8">
                    <div className="px-8 pb-4">
                        <label className="text-sm">체크인 날짜</label>
                        <input
                            type="date"
                    ...
                    <div className="px-8 pb-4">
                        <label className="text-sm">체크아웃 날짜</label>
                        <input
                            type="date"
                            className="w-full p-2 border rounded-md"
                            value={checkoutDate}
