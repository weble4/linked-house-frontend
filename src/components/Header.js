...
const Header = ({ isLoggedIn, onLogout }) => {
    ...
    useEffect(() => {
        setIsLoggedInLocalStorage(!!accessToken && tokenExpiration && currentTime < tokenExpiration);
    ...
    return (
        <header className="flex justify-between items-center p-4 border-b">
            <Link to="/" className="text-xl font-bold hover:text-blue-500">
                <img src={Logo} alt="로고" className="mx-8" style={{ width: "100px", height: "100px" }} />
            </Link>

            <nav className="flex space-x-4">
                {isLoggedInLocalStorage ? (
                    <div className="relative">
                        <button onClick={toggleProfileMenu} className="px-4 py-2 bg-blue-500 text-white rounded">
                            유저 정보
                        </button>
                        {isProfileMenuOpen && (
